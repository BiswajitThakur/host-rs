...
async fn block(app: &mut App) -> Result<(), reqwest::Error> {
    let mut downloads = Vec::<HashSet<String>>::with_capacity(app.data.hosts.social.urls.len());
    let mut capacity: usize = 0;
    for url in app.data.hosts.social.urls.iter() {
        if !url.is_enable {
            continue;
        };
    ...
    app.host.social.clear();
    app.host.social.extend(result);
    Ok(())
}
