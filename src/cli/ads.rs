...
async fn block(app: &mut App) -> Result<(), reqwest::Error> {
    let mut downloads = Vec::<HashSet<String>>::with_capacity(app.data.hosts.ads.urls.len());
    let mut capacity: usize = 0;
    for url in app.data.hosts.ads.urls.iter() {
        if !url.is_enable {
            continue;
        };
    ...
    if &result == &app.host.ads {
        return Ok(());
    };
    app.host.ads.clear();
    app.host.ads.extend(result);
    Ok(())
}
